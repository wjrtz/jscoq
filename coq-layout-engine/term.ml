...
let ly_cast (s : constr_expr Glob_term.cast_type) =
  match s with
  | Glob_term.CastConv _ ->
    xxx "castconv"
  | Glob_term.CastVM _ ->
    xxx "castvm"
  | Glob_term.CastNative _ ->
    xxx "castnative"
...
let ly_notation key args =
  let unpl, level = Ppextend.find_notation_printing_rule None key in
  ly_hunks unpl args
